...
var host    = args.host || process.env.KERNELPULSE_SERVER_HOST || '0.0.0.0'
var port    = args.port || process.env.KERNELPULSE_SERVER_PORT || 80

server.listen(port, host, function() {
  console.log('KernelPulse Server Started on ' + host + ':' + port + '!');
})
...
wsServer.on('request', function(request) {
  ...
  wsClient.on('message', function(wsReq) {
    ...
    var sendDataToClient = function(code, output) {
      if (code === 0) {
        var wsResponse = JSON.stringify({ moduleName: moduleName, output: output.join('') })
