...
issue() {
  ...
  if [ -f "$issueFile" ]; then
    result=$($CAT "$issueFile" | $SED 's/\\[a-z]//g' | $TR -d '\n' | $SED 's/^ *//;s/ *$//')
    $ECHO "\"$result\"" | _parseAndPrint
  else
    $ECHO '""' | _parseAndPrint
  fi
...
general_info() {
  ...
  $ECHO "{ \"os_distribution\": \"$os\", \"hostname\": \"$hostname\", \"Uptime\": \" $(displaytime ${uptime_seconds%.*}) \", \"Server Time\": \"$server_time\" }" | _parseAndPrint
}
...
scheduled_crons() {
    ...
    $CAT "${temp}" \
        | $AWK 'BEGIN {print "["} \
                    ...
                    " \"user\": \"" $6 "\", " \
                    " \"CMD\": \""} \
                        {for(i=7;i<=NF;++i) { f=$i; gsub("\"","\\\"",f); printf("%s ", f) } } \
                    {print "\" " \
                    "}," } \
                END {print "]"}' \
        | $SED 'N;$s/,\n//;P;D' \
